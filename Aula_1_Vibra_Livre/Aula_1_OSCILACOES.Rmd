...
<strong>Aula 1: Oscilações Livres</strong> — você está aqui &nbsp;|&nbsp;
...
> **Prévia — Aula 2:** O colapso da Ponte Tacoma (1940) é o exemplo mais emblemático de **ressonância forçada** na engenharia estrutural. A frequência natural de torção da ponte coincidiu com a frequência de excitação do vento, gerando amplificação dinâmica crescente até a ruptura. Este fenômeno será deduzido e quantificado na [Aula 2 — Oscilações Forçadas](https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_2_VIBRACAO_FORC/).
...
> **Conexão com a Aula 3 — Instrumentação:** A Eq$\eqref{eq:4.2}$ é o princípio físico de funcionamento de qualquer **acelerômetro**: o sensor mede $a(t)$ diretamente, e o deslocamento é recuperado por $x(t) = -a(t)/\omega_0^2$. Na [Aula 3 — Processamento de Sinais](https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_3_PROCESS_SINAIS/), o sensor **MPU6050** (embarcado em um ESP32) utiliza exatamente esta relação para monitorar vibrações de um motor — os mesmos parâmetros $m$, $k$ e $\omega_0$ desta equação determinam as frequências que aparecem nos espectros FFT medidos em tempo real.
...
> **Conexão com a Aula 3 — Espectro:** A frequência $\omega$ da Eq$\eqref{eq:43}$ é a **frequência natural amortecida** $\omega_d$. No domínio da frequência (Aula 3), o pico do espectro FFT de um sistema subamortecido aparece em $f_d = \omega_d/(2\pi)$ — ligeiramente **abaixo** de $f_0 = \omega_0/(2\pi)$. Além disso, a **largura do pico espectral** na potência de meia altura ($-3\,\text{dB}$) é proporcional a $\gamma$: quanto maior o amortecimento, mais largo e baixo é o pico. Esse é o método experimental para identificar $\omega_0$ e $\gamma$ a partir de medições — explorado na [Aula 3](https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_3_PROCESS_SINAIS/).
...
<span style="color:#888;font-size:0.88em;font-style:italic;">Aula 1 de 3 — Oscilações Livres</span>
<a href="https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_2_VIBRACAO_FORC/" target="_blank" style="background:#1e9aa6;color:white;padding:8px 20px;border-radius:4px;text-decoration:none;font-size:0.88em;font-weight:600;">Aula 2: Oscilações Forçadas →</a>
</div>
