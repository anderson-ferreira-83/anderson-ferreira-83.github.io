...
<strong>Aula 2: Oscilações Forçadas</strong> — você está aqui &nbsp;|&nbsp;
<a href="https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_3_PROCESS_SINAIS/" target="_blank">Aula 3: Processamento de Sinais →</a>
</div>
...
```{r ex1-forcada, echo=FALSE, fig.cap='Mecanismo de manivela: ilustra como máquinas rotativas convertem movimento circular em força periódica, fonte clássica de vibração forçada.', out.width='38%', fig.align='center'}
knitr::include_graphics('motor_desbalanceado.gif')
```

```{r ex2-forcada, echo=FALSE, fig.cap='Amortecedor de massa sintonizada (TMD) em edifício: o sistema massa-mola-amortecedor controla a resposta estrutural sob excitação dinâmica (sísmica ou vento).', out.width='38%', fig.align='center'}
knitr::include_graphics('edificio_sismico.gif')
```

```{r ex3-forcada, echo=FALSE, fig.cap='Oscilador mola-pêndulo com diferentes níveis de amortecimento: representa a resposta de estruturas (pontes, viadutos) sob carregamentos periódicos.', out.width='38%', fig.align='center'}
knitr::include_graphics('ponte_forcada.gif')
```

```{r ex4-forcada, echo=FALSE, fig.cap='Tubo acústico aberto em ressonância: a coluna de ar vibra em sua frequência natural quando excitada — princípio análogo ao ressonador de Helmholtz.', out.width='38%', fig.align='center'}
...
> **Vínculo com a Aula 1:** $x_h(t)$ é exatamente a solução deduzida na [Aula 1 — Oscilações Livres](https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_1_OSCILACOES/). Para o caso subamortecido ($\zeta < 1$): $x_h(t) = e^{-\zeta\omega_0 t}[D_1\cos(\omega_d t)+D_2\sin(\omega_d t)]$, onde $\omega_d = \omega_0\sqrt{1-\zeta^2}$ é a frequência amortecida. O forçamento externo **acrescenta** $x_p(t)$ a essa resposta já conhecida — daí o nome *superposição*.
...
```{r ex5-sistema-forcado, echo=FALSE, fig.cap='Sistema massa-mola com excitação harmônica externa $F(t) = F_0\\cos(\\omega t)$.', out.width='52%', fig.align='center'}
...
```{r ex6-batimento, echo=FALSE, fig.cap='Fenômeno de batimento: amplitude modulada quando $\\omega \\approx \\omega_0$.', out.width='68%', fig.align='center'}
...
```{r ex7-ressonancia, echo=FALSE, fig.cap='Ressonância: amplitude crescendo sem limite quando $\\omega = \\omega_0$. Compare com a queda da Ponte Tacoma da Aula 1.', out.width='68%', fig.align='center'}
...
```{r ex8-sistema-amortecido, echo=FALSE, fig.cap='Sistema massa-mola-amortecedor com excitação harmônica.', out.width='52%', fig.align='center'}
...
A Eq$\eqref{eq:22}$ conecta diretamente com a notação da **Aula 1**, onde o coeficiente de amortecimento foi definido como $\gamma = b/m\ [\text{s}^{-1}]$. A razão de amortecimento é simplesmente $\zeta = \gamma/(2\omega_0)$ — a versão **adimensional** de $\gamma$, normalizada pela frequência natural. Enquanto $\gamma$ depende dos parâmetros físicos brutos do sistema, $\zeta$ permite comparar o nível de amortecimento de sistemas com frequências naturais completamente diferentes — fundamental para a análise universal do Diagrama de Bode desta seção.
...
> **Conexão com a Aula 3 — FRF Experimental:** O FAD $H(r,\zeta)$ da Eq$\eqref{eq:35}$ é a versão **teórica** da **Função de Resposta em Frequência (FRF)**, central ao diagnóstico experimental de máquinas. Na prática, a FRF é estimada como a razão entre o espectro FFT da resposta e o da excitação: $\text{FRF}(f) = |Y(f)|/|X(f)|$. O **Diagrama de Bode** desta seção — amplitude $H$ e fase $\phi$ em função de $r$ — é exatamente o que o pipeline IoT da [Aula 3](https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_3_PROCESS_SINAIS/) mede ao analisar os espectros do sensor MPU6050. Em particular: o pico em $r_{res} = \sqrt{1-2\zeta^2}$ (Eq$\eqref{eq:38}$) corresponde ao pico dominante nos espectros calibrados do motor; e o valor $H_{max} = 1/(2\zeta)$ em $r=1$ determina o quanto a amplitude de vibração excede o valor estático — informação crítica para o diagnóstico de falha.
...
<a href="https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_1_OSCILACOES/" target="_blank" style="background:#163042;color:white;padding:8px 20px;border-radius:4px;text-decoration:none;font-size:0.88em;font-weight:600;">← Aula 1: Oscilações Livres</a>
<span style="color:#888;font-size:0.88em;font-style:italic;">Aula 2 de 3 — Oscilações Forçadas</span>
<a href="https://anderson-henrique-rodrigues-ferreira.shinyapps.io/Aula_3_PROCESS_SINAIS/" target="_blank" style="background:#1e9aa6;color:white;padding:8px 20px;border-radius:4px;text-decoration:none;font-size:0.88em;font-weight:600;">Aula 3: Processamento de Sinais →</a>
</div>
